...
- Integration configs in `integrations/` — ANSI-slot based so they work for every theme.
  `integrations/terminal-theme/` is a satellite generator (`theme.toml` → Ghostty theme,
  kitty conf, iTerm2 dynamic profile, Warp yaml, shell palette) for a *terminal* look
  (Matte Black HC) that hosts the pi themes. It is a **consumer of** the family, not a
  member: it reads its accents from `themes/pi/*.json`, adds no flavor under `palette/`
  and no export under `themes/`, and writes only into the user's config dirs
  (`make terminal-theme`, with backups + rollback). Its `palette.json` is generated.
...
- The six phosphor pi themes, `themes/pi/reckoner-*.json`. These are **sources,
  not outputs** — see the second pipeline below. One exception inside them:
  `themes/pi/derive_panels.py` owns exactly the nine `pa*Bg` vars (the panel
  backgrounds), deriving them against the terminal surface (THEMES.md "Surface
  rule"); identity colours, `paDiffText` and `paDim` stay hand-held. Because
  `paSelectedBg` also feeds the phosphor exports' selection colour and ANSI 0,
  those two export values are derived from a pi panel colour.
- The `-sol` light twins in `themes/pi/` and `scripts/generate_sol.py`, which
  derives them from the dark keepers (hue-preserving paper ladders, WCAG gates
  vs worst-case terminal backgrounds; `amanecer-sol` hand-authored from
  upstream Rosé Pine Dawn). Regenerate with
  `python3 scripts/generate_sol.py`; never hand-edit a generated `-sol`.
...
`archive/` for reference but are not part of the public contract. Seven pi
themes archived in the 2026-08-26 consolidation live in `archive/themes/pi/`:
amnesiac, anochecer, atardecer, madrugada, ocaso, veridis, voyager.
...
across all nine pi themes, and a 3:1 floor on every ANSI slot in the phosphor
...
`scripts/generate.py` derives the thinking ramp for the flavors from three facts: the
