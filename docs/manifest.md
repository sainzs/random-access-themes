...
- Integration configs in `integrations/` — ANSI-slot based so they work for every theme;
  `integrations/terminal-theme/` is a satellite generator (`theme.toml` → terminal configs)
  that reads its accents from `themes/pi/*.json` and is gated by `make terminal-theme`
