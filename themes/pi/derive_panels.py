...
# Only the six hand-authored dark tubes are ours to derive. Skipped: every light
# theme (mediodia and the generated `-sol` twins — detected by bg0 luminance),
# aerodynamic (GENERATED by scripts/generate.py, its own #000000 page) and
# amanecer (an upstream port whose colours are not ours to tune).
SKIP = {"mediodia", "aerodynamic", "random-access-theme", "amanecer"}

def _lum(h):
    def lin(c): return c / 12.92 if c <= 0.04045 else ((c + 0.055) / 1.055) ** 2.4
    r, g, b = (lin(c / 255) for c in rgb(h)); return 0.2126 * r + 0.7152 * g + 0.0722 * b
...
def derive(path, surface, dry):
    ...
    name = theme["name"]
    if name in SKIP or name.endswith("-sol"): return None
    res = resolver(theme)
    if _lum(res("bg0")) >= 0.5: return None      # light page: not a dark-surface theme
    colors = theme["colors"]
